...
echo '=================================================='
echo
echo "Step 0) If you already have Client ID and Client Secret, skip to Step 5."
echo
echo -n "Skip (y/n) [n]: "
read skip
if [ "$skip" == "y" ]; then
	echo
	echo "Steps 1-4 skipped"
else
	echo
	echo To begin, you will need to acquire Client ID and Client Secret.
	echo
	echo "Step 1) visit https://console.developers.google.com/apis/credentials?project=mythtv-file-sharing&authuser=0"
	echo
	echo "Step 2) Click on Create credentials and select OAuth client ID"
	echo
	echo "Step 3) Select Web application and add ${default_redirect_uri} to Authorized redirect URIs"
	echo
	echo "Note: Don't forget to add ${default_redirect_uri} as the Redirect URI!!"
	echo
	echo "Step 4) Click Create"
fi

echo
echo "Step 5) Once you have the credentials generated in your Browser window, enter those credentials here now."
echo
...
echo
echo "Step 6) Visit this Access Permit URL (one time only):"
echo
...
echo "Step 7) Please paste in the URL of the visited page"
